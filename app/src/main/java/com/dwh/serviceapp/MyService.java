...
public class MyService extends Service {
    ...
    private class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            Log.i("MyService", "ServiceHandler -> handleMessage");
            if (msg.what == RECEIVE_MESSAGE_CODE) {
                Bundle data = msg.getData();
                if (data != null) {
                    String str = data.getString("msg");
                    Log.i("MyService", "MyService收到客户端如下信息: " + str);
                }
                //通过Message的replyTo获取到客户端自身的Messenger
                //Service可以通过它向客户端发送消息
                clientMessenger = msg.replyTo;
                if (clientMessenger != null) {
                    Log.i("MyService", "MyService向客户端回信");
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        Log.e("MyService", "MyService向客户端发送信息失败: " + e.getMessage());
                    }
                }
            }
        }
    ...
    public IBinder onBind(Intent intent) {
        Log.i("MyService", "MyServivce -> onBind");
